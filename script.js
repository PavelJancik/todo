...
function show_info_clicked(show_btn) {
  ...
  $(show_btn).text(
    $(show_btn).text() == "Show Info" ? "Hide Info" : "Show Info",
  );
}
...
async function button_clicked(status, item) {
  ...
  await $item
    .slideToggle(500, function () {
      $item.prependTo(`#${status}`);
      if ($item.attr("data-preview") != "false")
        $item.find(".content_wrap").hide();
      $item.slideToggle(500).css("height", "auto");
...
function create_new_item() {
  ...
  let item = `<div id="${item_id}" class="item testing_item" data-status="todo" data-changed="${new Date().valueOf()}" data-preview='false' 
                   onmouseover="handle_mouse_over($(this))" onmouseleave="handle_mouse_leave($(this))">
...
function save_item_change(item, update_data_changed = true) {
  ...
  localStorage.setItem(
    $(item).attr("id"),
    JSON.stringify($(item).wrap("<div/>").parent().html()),
  );
}
function load_item_from_localStorage(item_id) {
  let item_html = JSON.parse(localStorage.getItem(item_id));
  let status = $($.parseHTML(item_html)).attr("data-status"); // .data('status')
  $(`#${status}`).append(item_html);
  if ($(`#${item_id}`).attr("data-preview") == "true")
    shorten_item($(`#${item_id}`));
}
function sort_items() {
  $(".container").each(function () {
    $(this)
      ...
      .sort(function (x, y) {
        return (
          parseInt(x.getAttribute("data-changed")) -
          parseInt(y.getAttribute("data-changed"))
        );
      })
      .forEach((item) => {
        $(this).prepend($(item));
...
function empty_trash_bin() {
  if (
    confirm(
      `Warning: ${
        $("#trash_bin .item").length
      } items from Trash Bin will be deleted! Continue?`,
    )
  ) {
...
function extend_item(item) {
  $(item)
    ...
    .slideDown(300, function () {
      ...
      $(item).find(".content_wrap").css("height", "auto");
...
function toggle_item_preview(item) {
  if (
    $(item).attr("data-preview") == "true" ||
    $(item).attr("data-preview") == "temp"
  )
    extend_item(item);
  else shorten_item(item);
}
function handle_mouse_over(item) {
  if ($(item).attr("data-preview") == "true") {
    $(item).attr("data-preview", "temp");
    $(item).find(".content_wrap").delay(50).slideDown(300);
  }
}
function handle_mouse_leave(item) {
  setTimeout(() => {
    if ($(item).attr("data-preview") == "temp") {
      $(item)
        .find(".content_wrap")
        .slideUp(300, function () {
          $(item).attr("data-preview", "true");
